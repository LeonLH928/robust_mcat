...
class Robust_MCAT(nn.Module):
    def __init__(self, use_condition=False, generator=False, g_model_type='ldvae', decoder_mode='specific', alpha_surv=0., wsi_encoding_dim=1024, fusion='concat', omic_sizes=[100, 200, 300, 400, 500, 600], n_classes=2,
        ...
        if generator: 
            ...
            self.wsi_loss = nn.CrossEntropyLoss()
    ...
    def forward(self, omic_missing=False, **kwargs):
        ...
        if kwargs['train']:       
            if self.generator:
                ...
                encode_wsi_loss = self.wsi_loss(logits_wsi, label)
                probs = torch.softmax(logits_wsi, dim=1)
                risk_wsi = probs[:, 0].detach().cpu().numpy()
        ...
        if self.generator:
            return logits, Y_prob, Y_hat, attention_scores, all_loss
